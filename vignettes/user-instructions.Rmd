...
* **Step 3. Make location table** 
  ...
  + **Step 3.2 (optional). Link to existing NaTRON locations** 
...
```{r, eval=FALSE}
myLocationTable <- location_check(data = setesdal, conn = MyConnection)
...
### Step 3.1 (optional)
If you know or suspect that the locations in your data already exists in NaTRON then you probably want to reuse the locations and the unique locationIDs so that you don't break up time series data for example. 

Check for pre-existing localities in Natron:
To prevent unwanted duplicates in NaTRON, and to ensure your data is correctly linked to any existing data, you are asked to re-use locations already present in the database if possible.
```location_check()``` scans NaTRON for locations that are identical to, or within a given radius if, the locations in the dataset that is to be uploaded. Firsrt, set the radius in meters:
...
If ```location_check()``` returns NaTRON locations within the defined radius, ```get_new_loc()```  will let you choose whether to use these preexisting locations and their unique *locationID*. This makes updating time series data much more standardised.



### The `location_check` function takes three arguments:
