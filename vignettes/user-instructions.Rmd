...
NaTRON has a _Taxa_ table with information about most of the species you can encunter in Norway. The occurence table needs to link to this table via the _taxonID_ field. To accomplish this the species names in your dataset needs to be spellt correctly and to be an accepted name for that species according to Artsdatabanken. 
...
This final check will compare your scientific species names against the NaTron taxa register and perform a fuzzy match to return both perfect and im-perfect matches. Look through the dataframe to find mistakes and change the species names in the raw data if needed. If a good match for your species cannot be found you need to add it to NaTron. This is for now done manually here: https://natron.vm.ntnu.no/datacollection/ 
