...
This step scans NaTRON for locations that are identical to, or within a given radius if, the locations in the dataset that is to be uploaded. If ```location_check()``` returns matching locations that already exists in NaTRON, ```get_new_loc()```  will let you choose whether to use these preexisting locations and their unique *locationID*. This makes updating time series data much more standardised.
